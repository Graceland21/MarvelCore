...
# =========================
TOKEN = os.getenv("8704948433:AAHuDzufWTXEvIpZyMirtb-_gxXdVVxbNiI")
CHAT_ID = os.getenv("-1002497463613")

bot = Bot(token=8704948433:AAHuDzufWTXEvIpZyMirtb-_gxXdVVxbNiI)
...
async def send_signal(pair):
    ...
    signal_time = entry_time - timedelta(minutes=TIMEFRAME)
...
"""
    ...
    return entry_price, direction, entry_time
...
async def main():
    ...
    while True:
        try:
            for pair in PAIRS:
                entry_price, direction, entry_time = await send_signal(pair)

                if entry_price:
                    asyncio.create_task(
                        check_result(pair, entry_price, direction, entry_time)
                    )
